...
// Variables estáticas para detectar pulsación única
...
static bool key4 = false;
static bool keyG = false;
...
int main()
{
	...
	naveTexture = Texture("Textures/nave.tga");
	...
	Nave_M = Model();
	Nave_M.LoadModel("Models/nave10.obj");
	Ala_M = Model();
	Ala_M.LoadModel("Models/ala.obj");
	AlaIzq_M = Model();
	AlaIzq_M.LoadModel("Models/alaizq10.obj");
	...
	KeyFrame[5].movAvion_x = 2.0f;
	KeyFrame[5].movAvion_y = 0.0f;
	KeyFrame[5].giroAvion = 180.0f;

	KeyFrame[6].movAvion_x = 1.0f;
	KeyFrame[6].movAvion_y = 2.0f;
	...
	while (!mainWindow.getShouldClose())
	{
		...
		if (play)
		{
			anguloAla = sin(glfwGetTime() * 8.0f) * 12.0f;
		}
		else
		{
			anguloAla = 0.0f;
		...
		// ALA Derecha
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.13f, 0.19f, -0.15f));

		// Punto de pivote del ala
		...
		// ALA Izquierda
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.13f, 0.19f, 0.3f));
...
void inputKeyframes(bool* keys)
{
	...
	if (keys[GLFW_KEY_SPACE])
	{
		if (!keySpace)
		{
			if (play == false && (FrameIndex > 1))
			{
				...
				playIndex = 0;
				i_curr_steps = 0;
				printf("Animación iniciada\n");
			}
			else
			{
				play = false;
				printf("Animación detenida\n");
			}
			keySpace = true;
		}
	}
	...
	if (keys[GLFW_KEY_G])
	{
		if (!keyG)
		{
			...
			// Para evitar valores como 360, 540, etc.
			if (giroAvion >= 360.0f)
				giroAvion = 0.0f;
