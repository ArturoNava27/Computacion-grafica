...
// Modelo GLB con animacion esqueletal.
...
int main()
{
	...
	// Jayce GLB animado. Coloca jayce.glb en la carpeta Models.
...
// Asi no dependen de puntos guardados y visualmente se sienten como vuelo normal.
	...
	// Jayce con animacion esqueletal desde GLB.
	// Se deja como sistema aparte para no tocar los Model .obj normales.
	// Recorrido con tecla 3:
	// 0) Al iniciar queda estatico en jayceInicio, sin actualizar la animacion esqueletal.
	// 1) Al presionar 3 se desplaza de jayceInicio a jayceFinal.
	// 2) La trayectoria fisica usa una parabola: sube poco al principio y cae mas rapido.
	// 3) La animacion GLB se redistribuye: aterriza cerca del 75% del trayecto
	//    y el ultimo 25% se usa para que Jayce se levante en el punto final.
	// 4) Al llegar se queda estatico en jayceFinal.
	...
	int faseRecorridoJayce = 0; // 0 = inicio quieto, 1 = salto/caida animado, 2 = final quieto
	float tiempoRecorridoJayce = 0.0f;
	float duracionJayceRecorrido = 12.00f;

	// Ajustes del salto de Jayce.
	// progresoFisicoAterrizaje indica en que parte del tiempo total ya debe tocar el suelo.
	// El resto del tiempo se queda en jayceFinal mientras la animacion termina de levantarse.
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// Tecla 3: inicia el recorrido completo de Jayce.
		// Antes de presionar 3, Jayce permanece estatico en la posicion inicial.
		...
		if (recorridoJayceActivo && faseRecorridoJayce == 1)
		{
			...
			// Movimiento fisico:
			// - De 0.00 a 0.75 se mueve por el aire.
			// - Al 0.75 ya toca el suelo en jayceFinal.
			// - De 0.75 a 1.00 se queda en el punto final mientras termina de levantarse.
			float progresoVueloJayce = Clamp01(progresoJayce / progresoFisicoAterrizajeJayce);
			float avanceHorizontalJayce = pow(progresoVueloJayce, 0.72f); // avanza un poco mas al inicio
			...
			{
				// Subida corta y suave: primero se adelanta un poco mientras sube.
			...
			{
				// Caida con gravedad mas pesada: al principio cae poco, luego acelera fuerte.
			...
			{
				// Ya aterrizo: no sigue moviendose, solo termina la pose de levantarse.
				posicionJayce = jayceFinal;
			}

			// Redistribucion de la animacion:
			// La parte de caida/impacto llega a progresoAnimAterrizajeJayce cuando
			// el movimiento fisico va por progresoFisicoAterrizajeJayce.
			// El tramo restante se reserva para que Jayce se levante ya en el suelo.
		...
		/*
		// Menu para mover Jayce.
		// IMPORTANTE: se reutiliza el mismo menu base del codigo.
		// Esta comentado porque ahora Jayce usa el recorrido con tecla 3.
		// Si despues necesitas reacomodarlo, se reactiva este mismo bloque.
		// Flechas / H L J K / Numpad 4 6 8 2 = mover X/Z
		// Q / E / Numpad 7 9 = subir/bajar
		// + / - = escala
		// R / T / Numpad 1 3 = rotar
		// P = imprimir posicion en consola
		bool movioJayce = false;
		static bool teclaPJayceLiberada = true;
		bool* keysJayce = mainWindow.getsKeys();

		if (keysJayce[GLFW_KEY_LEFT] || keysJayce[GLFW_KEY_H] || keysJayce[GLFW_KEY_KP_4])
		{
			posicionJayce.x -= velocidadMoverJayce * deltaTime;
			movioJayce = true;
		}
		if (keysJayce[GLFW_KEY_RIGHT] || keysJayce[GLFW_KEY_L] || keysJayce[GLFW_KEY_KP_6])
		{
			posicionJayce.x += velocidadMoverJayce * deltaTime;
			movioJayce = true;
		}
		if (keysJayce[GLFW_KEY_UP] || keysJayce[GLFW_KEY_J] || keysJayce[GLFW_KEY_KP_8])
		{
			posicionJayce.z -= velocidadMoverJayce * deltaTime;
			movioJayce = true;
		}
		if (keysJayce[GLFW_KEY_DOWN] || keysJayce[GLFW_KEY_K] || keysJayce[GLFW_KEY_KP_2])
		{
			posicionJayce.z += velocidadMoverJayce * deltaTime;
			movioJayce = true;
		}

		if (keysJayce[GLFW_KEY_Q] || keysJayce[GLFW_KEY_KP_7])
		{
			posicionJayce.y += velocidadAlturaJayce * deltaTime;
			movioJayce = true;
		}
		if (keysJayce[GLFW_KEY_E] || keysJayce[GLFW_KEY_KP_9])
		{
			posicionJayce.y -= velocidadAlturaJayce * deltaTime;
			movioJayce = true;
		}

		if (keysJayce[GLFW_KEY_EQUAL] || keysJayce[GLFW_KEY_KP_ADD])
		{
			escalaJayce += velocidadEscalaJayce * deltaTime;
			if (escalaJayce > escalaMaximaJayce) escalaJayce = escalaMaximaJayce;
			movioJayce = true;
		}
		if (keysJayce[GLFW_KEY_MINUS] || keysJayce[GLFW_KEY_KP_SUBTRACT])
		{
			escalaJayce -= velocidadEscalaJayce * deltaTime;
			if (escalaJayce < escalaMinimaJayce) escalaJayce = escalaMinimaJayce;
			movioJayce = true;
		}

		if (keysJayce[GLFW_KEY_R] || keysJayce[GLFW_KEY_KP_1])
		{
			rotacionJayceY -= velocidadRotacionJayce * deltaTime;
			movioJayce = true;
		}
		if (keysJayce[GLFW_KEY_T] || keysJayce[GLFW_KEY_KP_3])
		{
			rotacionJayceY += velocidadRotacionJayce * deltaTime;
			movioJayce = true;
		}

		if (keysJayce[GLFW_KEY_P])
		{
			if (teclaPJayceLiberada)
			{
				printf("Jayce pos = glm::vec3(%.2ff, %.2ff, %.2ff); | escala = %.2ff | rotY = %.2ff\n",
					posicionJayce.x, posicionJayce.y, posicionJayce.z, escalaJayce, rotacionJayceY);
				teclaPJayceLiberada = false;
			}
		}
		else
		{
			teclaPJayceLiberada = true;
		}

		tiempoImpresionJayce += deltaTime;
		if (movioJayce && tiempoImpresionJayce >= 0.35f)
		{
			printf("Jayce pos = glm::vec3(%.2ff, %.2ff, %.2ff); | escala = %.2ff | rotY = %.2ff\n",
				posicionJayce.x, posicionJayce.y, posicionJayce.z, escalaJayce, rotacionJayceY);
			tiempoImpresionJayce = 0.0f;
		}
		*/
		...
		*/
		...
		// Jayce GLB con animacion esqueletal.
		// Usa shader aparte para no tocar el shader de los OBJ normales.
		if (Jayce_M.IsLoaded())
		{
