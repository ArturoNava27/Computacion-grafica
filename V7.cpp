...
// Nuevos modelos integrados AS, Paris, Terry
Model Piernad_as_M;
Model Piernai_as_M;
Model ColaAs_M;
Model As_M;
...
// Variables procedurales para AS, Paris y Terry
...
// Posiciones calculadas para que no choquen cerca de la puerta (x=61.7, z=-74.7)
float posX_AS = 40.0f;
float posZ_AS = -40.0f;
float anguloAS = 0.0f;
int ladoAS = 0;
...
int main()
{
	...
	// Lampara_M eliminado: no se carga ni se renderiza.
	// Lampara_M = Model();
	// Lampara_M.LoadModel("Models/Lampara.obj");
	Barco_M = Model();
	Barco_M.LoadModel("Models/bote.obj");

	// Carga de modelos procedimentales (AS, Paris, Terry)
	Piernad_as_M = Model();
	...
	// Tren/Vias eliminados: no se cargan ni se renderizan.
	// Tren_M = Model();
	// Tren_M.LoadModel("Models/SteamTrain.fbx");
	// Vias_M = Model();
	// Vias_M.LoadModel("Models/trilho.obj");
	Helice_M = Model();
	...
	// Banca temporal al centro del mundo para usarla despues.
	glm::vec3 posicionBanca = glm::vec3(0.0f, -2.0f, 0.0f);
	float rotacionBancaY = 0.0f;
	float escalaBanca = 1.0f;

	// Jerarquia base para Gabu. Todas sus piezas son hijas de esta matriz.
	...
	// Volcan colocado con el ultimo valor impreso.
	...
	// Variables para mover la Fabrica con el menu reutilizable.
	...
	// Variables para mover la Puerta con el menu reutilizable.
	...
	// Variables para mover la piramide / Centro de visitantes con el menu reutilizable.
	...
	// Variables para mover el modelo geodesico con el menu reutilizable.
	...
	// Reptil colocado con el ultimo valor impreso.
	...
	// Rex con animacion ciclica fluida.
	...
	// Menu de Rex comentado/desactivado. Se conservan variables por si se reactiva despues.
	...
	while (!mainWindow.getShouldClose())
	{
		...
		{
			// El reptil solo se anima cuando la puerta esta abriendose/abierta.
			// Cuando la puerta se cierra, vuelve a quedarse estatico al inicio de su animacion.
		...
		*/
		...
		/*
		// Menu de teclas reutilizable para mover el modelo geodesico.
		// Flechas / H J K / Numpad 4 6 8 2 = mover X/Z
		// Q / E / Numpad 7 9 = subir/bajar
		// + / - = escala
		// R / T / Numpad 1 3 = rotar
		// P = imprimir posicion en consola
		bool movioGeodesico = false;
		static bool teclaPGeodesicoLiberada = true;
		bool* keysGeodesico = mainWindow.getsKeys();

		if (keysGeodesico[GLFW_KEY_LEFT] || keysGeodesico[GLFW_KEY_H] || keysGeodesico[GLFW_KEY_KP_4])
		{
			posicionGeodesico.x -= velocidadMoverGeodesico * deltaTime;
			movioGeodesico = true;
		}
		if (keysGeodesico[GLFW_KEY_RIGHT] || keysGeodesico[GLFW_KEY_KP_6])
		{
			posicionGeodesico.x += velocidadMoverGeodesico * deltaTime;
			movioGeodesico = true;
		}
		if (keysGeodesico[GLFW_KEY_UP] || keysGeodesico[GLFW_KEY_J] || keysGeodesico[GLFW_KEY_KP_8])
		{
			posicionGeodesico.z -= velocidadMoverGeodesico * deltaTime;
			movioGeodesico = true;
		}
		if (keysGeodesico[GLFW_KEY_DOWN] || keysGeodesico[GLFW_KEY_K] || keysGeodesico[GLFW_KEY_KP_2])
		{
			posicionGeodesico.z += velocidadMoverGeodesico * deltaTime;
			movioGeodesico = true;
		}

		if (keysGeodesico[GLFW_KEY_Q] || keysGeodesico[GLFW_KEY_KP_7])
		{
			posicionGeodesico.y += velocidadAlturaGeodesico * deltaTime;
			movioGeodesico = true;
		}
		if (keysGeodesico[GLFW_KEY_E] || keysGeodesico[GLFW_KEY_KP_9])
		{
			posicionGeodesico.y -= velocidadAlturaGeodesico * deltaTime;
			movioGeodesico = true;
		}

		if (keysGeodesico[GLFW_KEY_EQUAL] || keysGeodesico[GLFW_KEY_KP_ADD])
		{
			escalaGeodesico += velocidadEscalaGeodesico * deltaTime;
			if (escalaGeodesico > escalaMaximaGeodesico)
				escalaGeodesico = escalaMaximaGeodesico;
			movioGeodesico = true;
		}
		if (keysGeodesico[GLFW_KEY_MINUS] || keysGeodesico[GLFW_KEY_KP_SUBTRACT])
		{
			escalaGeodesico -= velocidadEscalaGeodesico * deltaTime;
			if (escalaGeodesico < escalaMinimaGeodesico)
				escalaGeodesico = escalaMinimaGeodesico;
			movioGeodesico = true;
		}

		if (keysGeodesico[GLFW_KEY_R] || keysGeodesico[GLFW_KEY_KP_1])
		{
			rotacionGeodesicoY -= velocidadRotacionGeodesico * deltaTime;
			movioGeodesico = true;
		}
		if (keysGeodesico[GLFW_KEY_T] || keysGeodesico[GLFW_KEY_KP_3])
		{
			rotacionGeodesicoY += velocidadRotacionGeodesico * deltaTime;
			movioGeodesico = true;
		}

		if (keysGeodesico[GLFW_KEY_P])
		{
			if (teclaPGeodesicoLiberada)
			{
				printf("Geodesico pos = glm::vec3(%.2ff, %.2ff, %.2ff); | escala = %.2ff | rotY = %.2ff\n",
					posicionGeodesico.x, posicionGeodesico.y, posicionGeodesico.z, escalaGeodesico, rotacionGeodesicoY);
				teclaPGeodesicoLiberada = false;
			}
		}
		else
		{
			teclaPGeodesicoLiberada = true;
		}

		tiempoImpresionGeodesico += deltaTime;
		if (movioGeodesico && tiempoImpresionGeodesico >= 0.35f)
		{
			printf("Geodesico pos = glm::vec3(%.2ff, %.2ff, %.2ff); | escala = %.2ff | rotY = %.2ff\n",
				posicionGeodesico.x, posicionGeodesico.y, posicionGeodesico.z, escalaGeodesico, rotacionGeodesicoY);
			tiempoImpresionGeodesico = 0.0f;
		}
		...
		*/
		...
		/*
		Menu de teclas reutilizable para mover Rex.
		Actualmente queda comentado/desactivado porque Rex ya tiene animacion ciclica automatica.
		Cuando se necesite moverlo otra vez, se reactiva este mismo formato de menu.
		...
		*/
		...
		// Menu de teclas reutilizable para Gabu.
		// Actualmente queda desactivado porque Gabu ahora se controla con WASD
		// desde la camara 3 en tercera persona.
		...
		float reboteGabu = 0.0f; // sin rebote para que las uniones no se despeguen del torso
		...
		// Marcha diagonal tipo animal:
		//  - delantera izquierda avanza junto con trasera derecha
		//  - delantera derecha avanza junto con trasera izquierda
		// Se mantiene un movimiento suave para evitar que las patas se abran hacia afuera.
		...
		// Banca al centro del mundo. Se deja independiente de las jerarquias principales.
		model = glm::mat4(1.0f);
		...
		// Gabu unido mediante jerarquia: todas las piezas cuelgan de la matriz gabu.
		color = glm::vec3(1.0f, 1.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		toffset = glm::vec2(0.0f, 0.0f);
		glUniform2fv(uniformTextureOffset, 1, glm::value_ptr(toffset));
		...
		// Se elimino la implementacion visual de luz en faros (triangulos/aura).

		if (Jayce_M.IsLoaded() || Vi_M.IsLoaded() || Zorro_M.IsLoaded() || Reptil_M.IsLoaded() || Rex_M.IsLoaded())
		{
		...
		// Se deja modelauxPuerta sin escala general para no deformar los offsets/pivotes de las hojas.
		...
		modelCuerpo = glm::translate(modelCuerpo, glm::vec3(posX_AS, 0.5f, posZ_AS)); // Subido para no atravesar el piso
		modelCuerpo = glm::rotate(modelCuerpo, glm::radians(anguloAS), glm::vec3(0.0f, 1.0f, 0.0f));
		modelCuerpo = glm::scale(modelCuerpo, glm::vec3(0.33f, 0.33f, 0.33f)); // Reducido a la mitad (aprox 1/3 del original)
		...
		modelCuerpoParis = glm::translate(modelCuerpoParis, glm::vec3(posX_PARIS, 0.5f, posZ_PARIS)); // Subido para no atravesar el piso
		modelCuerpoParis = glm::rotate(modelCuerpoParis, glm::radians(anguloPARIS), glm::vec3(0.0f, 1.0f, 0.0f));
		modelCuerpoParis = glm::scale(modelCuerpoParis, glm::vec3(0.33f, 0.33f, 0.33f)); // Reducido a la mitad (aprox 1/3 del original)
		...
		modelTerry = glm::translate(modelTerry, glm::vec3(100.0f, -2.0f, -140.0f)); // Bajado y alejado de la puerta
		modelTerry = glm::scale(modelTerry, glm::vec3(0.05f, 0.05f, 0.05f)); // Reducido a la mitad (1/20 de su tamaño original)
