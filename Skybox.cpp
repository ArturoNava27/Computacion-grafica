...
{
	DrawSkybox(viewMatrix, projectionMatrix, 1.0f, glm::vec3(1.0f, 1.0f, 1.0f));
}

void Skybox::DrawSkybox(glm::mat4 viewMatrix, glm::mat4 projectionMatrix, GLfloat intensidadSkybox, glm::vec3 tinteSkybox)
{
	...
	// Como la clase Shader del profe no expone directamente el ID del programa,
	// se toma el programa activo despues de UseShader para mandar los uniforms.
	GLint programaSkybox = 0;
	glGetIntegerv(GL_CURRENT_PROGRAM, &programaSkybox);
	GLuint uniformIntensidadSkybox = glGetUniformLocation(programaSkybox, "intensidadSkybox");
	GLuint uniformTinteSkybox = glGetUniformLocation(programaSkybox, "tinteSkybox");
	glUniform1f(uniformIntensidadSkybox, intensidadSkybox);
	glUniform3fv(uniformTinteSkybox, 1, glm::value_ptr(tinteSkybox));
